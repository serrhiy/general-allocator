...
void allocation() {
  ...
  memory1 =
      static_cast<int32_t*>(memory_realloc(memory1, sizeof(int32_t) * min * 2));
  memory2 =
      static_cast<int32_t*>(memory_realloc(memory2, sizeof(int32_t) * mid * 2));
  memory3 =
      static_cast<int32_t*>(memory_realloc(memory3, sizeof(int32_t) * big * 2));
  ...
  for (size_t index = 0; index < min / 2; index++)
    assert(memory1[index] == index);
  for (size_t index = 0; index < mid / 2; index++)
    assert(memory2[index] == index);
  for (size_t index = 0; index < big / 2; index++)
    assert(memory3[index] == index);
